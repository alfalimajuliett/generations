...
class SeedFeederBiennial(BaseModel):
    ...
    def seedbank(self, gen):
        ...
        if gen == 0:
            ...
            attrition_by_weevil =  math.e**(-
                (
                self.seed_weevil_attack_rate * self.seed_weevil(gen - 1)*self.seed_weevil_damage_function_shape) / (
                    1 + self.plant_dd_shape_par*(self.flower(gen - 1))))  #multiplied by fecundity
            ...
            return (probability_seeds_stay * previous_seeds) + (dens_dependent*self.maximum_plant_fecundity)/(1 +  self.plant_dd_shape_par*(self.flower(gen - 1)+self.rosette(gen-1)))*attrition_by_weevil
    ...
    def seed_weevil(self, gen):
        ...
        if gen == 0:
            ...
            return (self.flower(gen - 1) * self.seed_weevil(
                gen - 1) * self.seed_weevil_attack_rate * self.larval_survival) * math.e**(
                -(self.seed_weevil_attack_rate*self.maximum_plant_fecundity * self.seed_weevil(gen - 1)) / (1 +
                self.plant_dd_shape_par*(self.flower(gen-1))))
